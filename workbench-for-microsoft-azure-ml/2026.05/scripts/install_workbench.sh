...
echo "$d Fetching Posit Workbench 2026.05.1+225.pro10 $d"
# For non-development versions, download the deb package using apt-get
apt-get download "${package_name}=2026.05.1+225.pro10"
deb_file="$(pwd)/$(ls ${package_name}*.deb)"

# Install dependencies
# shellcheck disable=SC2046  # word splitting is intentional to expand the dependency list
...
# Install Workbench
echo "$d Install Posit Workbench 2026.05.1+225.pro10 $d"
